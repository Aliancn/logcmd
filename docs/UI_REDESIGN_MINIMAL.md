...
4. **任务模式交互**
   - `Enter` 查看实时日志仍为 TODO，需要将任务输出接入 LogViewMode（或新的 tail 视图）并在退出时回到任务模式。
   - 强制终止快捷键实现为 `k`，与设计稿中的 `X` 不一致，且缺乏终止后的状态反馈。
5. **日志查看链路**
   - 从任务模式进入日志查看、携带 PID 或 log path、以及浏览完毕后的返回路径尚未打通，需要补齐消息流和错误处理。
6. **测试与性能基准**
...
- [ ] 任务模式 `Enter` 打开实时日志（含返回路径），`x/X` 提供明确的停止/终止反馈
