...
var taskTailCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		return runTaskTail(args[0])
	},
}

func init() {
	taskTailCmd.Flags().BoolVarP(&taskTailFollow, "follow", "f", false, "实时跟踪日志输出")
	taskTailCmd.Flags().IntVarP(&taskTailLines, "lines", "n", 20, "显示最后几行日志")
	taskCmd.AddCommand(taskTailCmd)
}

func runTaskTail(idArg string) error {
	...
	tailArgs := []string{"-n", strconv.Itoa(taskTailLines)}
	if taskTailFollow {
	...
	// 处理中断信号，确保退出时不影响后台任务
	// 实际上，logcmd task tail 只是一个查看器，kill 掉它不会影响产生日志的 task 进程。
	// 这里直接运行 tail 命令即可，用户 Ctrl+C 会终止 logcmd task tail 和其子进程 tail。
