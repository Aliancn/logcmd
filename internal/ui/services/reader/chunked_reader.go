...
func (r *ChunkedReader) BuildIndex() error {
	...
	scanner.Buffer(buf, 1024*1024) // 最大1MB per line
	...
	for scanner.Scan() {
		...
		// 每indexInterval行记录一次偏移
		if lineNum%r.indexInterval == 0 {
			r.lineIndex = append(r.lineIndex, offset)
		}
	}
...
func (r *ChunkedReader) GetIndexProgress() float64 {
	...
	if len(r.lineIndex) == 0 {
		return 0.0
	}

	lastOffset := r.lineIndex[len(r.lineIndex)-1]
	return float64(lastOffset) / float64(r.fileSize)
}
